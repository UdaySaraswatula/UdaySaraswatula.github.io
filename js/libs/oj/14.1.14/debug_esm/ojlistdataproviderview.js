...
import { DataProviderRefreshEvent, FilterFactory } from 'ojs/ojdataprovider';
...
class ListDataProviderView {
    ...
    _filterResult(filterCriterion, items) {
        if (filterCriterion) {
            if (!filterCriterion.filter) {
                filterCriterion = FilterFactory.getFilter({ filterDef: filterCriterion });
            }
    ...
    _combineFilters(params) {
        ...
        if (filters.length == 0) {
            ...
            filterCriterion = FilterFactory.getFilter({ filterDef: { op: '$and', criteria: filters } });
        }
...
export default ListDataProviderView;

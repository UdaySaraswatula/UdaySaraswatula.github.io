...
function _getNativePickerDate(converter, isoString) {
  ...
  var valueParams;
  try {
    valueParams = IntlConverterUtils._dateTime(
      isoString,
...
oj.__registerWidget('oj.ojInputDate', $.oj.inputBase, {
  ...
  _ComponentCreate: function () {
    ...
    IntlConverterUtils._verifyValueMinMax(this.options.value, this.options.min, this.options.max);
  ...
  _setOption: function (key, value, flags) {
    ...
    if (key === 'disabled') {
      ...
      this._AfterSetOptionDisabledReadOnly('readOnly', EditableValueUtils.readOnlyOptionOptions);
  ...
  _cleanUpDateResources: function () {
    ...
    if (this._popUpDpDiv && isComponentInitialized(this._popUpDpDiv, 'ojPopup')) {
  ...
  _getMinMaxDateIso: function (minOrMax) {
    ...
    if (minMaxDateIso) {
      ...
      minMaxDateIso = IntlConverterUtils._minMaxIsoString(minMaxDateIso, dateIso);
    }
        ...
        for (var dRow = 0; dRow < numRows; dRow++) {
          ...
          if (weekDisplay !== 'none') {
            try {
              ...
              info(`The value of the InputDateTime element should be an ISOString,
  ...
  _validateDatetime: function (isoString, actionParam, doParseValue) {
    var valueParams;
    try {
      valueParams = IntlConverterUtils._dateTime(isoString, actionParam, doParseValue);
    } catch (e) {
      info(
...
setDefaultOptions({
  ojInputDate: {
    ...
    firstDayOfWeek: createDynamicPropertyGetter(function () {
      return getFirstDayOfWeek();
    }),

    dayWide: createDynamicPropertyGetter(function () {
      return getDayNames('wide');
    }),

    dayNarrow: createDynamicPropertyGetter(function () {
      return getDayNames('narrow');
    }),

    monthWide: createDynamicPropertyGetter(function () {
      return getMonthNames('wide');
    }),

    monthAbbreviated: createDynamicPropertyGetter(function () {
      return getMonthNames('abbreviated');
    }),

    datePicker: createDynamicPropertyGetter(function () {
      return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {}).datePicker;
    }),

    renderMode: createDynamicPropertyGetter(function () {
      return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {}).renderMode;
    }),

    keyboardEdit: createDynamicPropertyGetter(function () {
...
oj.__registerWidget('oj.ojInputTime', $.oj.inputBase, {
  ...
  _createWheelPicker: function () {
    ...
    if (this._isIndependentInput()) {
      ...
      if (pickerAttrs) {
        EditableValueUtils.setPickerAttributes(
  ...
  _timepickerShowing: function () {
    ...
    if (this._isIndependentInput()) {
      ...
      return (
        (picker && isComponentInitialized(picker, 'ojPopup') && picker.ojPopup('isOpen')) ||
        this._nativePickerShowing
      );
    }
    ...
    return (
      (widget._isShowingDatePickerSwitcher() &&
        picker &&
        isComponentInitialized(picker, 'ojPopup') &&
  ...
  _cleanUpTimeResources: function () {
    ...
    if (
      ...
      isComponentInitialized(this._popUpWheelPicker, 'ojPopup')
    ) {
  ...
  _showNativeTimePicker: function () {
    ...
    if (minDate) {
      ...
      var minDateProcessed = _getNativePickerDate(
        converter,
        IntlConverterUtils._minMaxIsoString(minDate, this._getIsoDateValue(converter))
      );
    ...
    if (maxDate) {
      ...
      var maxDateProcessed = _getNativePickerDate(
        converter,
        IntlConverterUtils._minMaxIsoString(maxDate, this._getIsoDateValue(converter))
      );
    ...
    function onTimePicked(cbDate) {
      ...
      if (cbDate) {
        ...
        var isoString = IntlConverterUtils._dateTime(self._getIsoDateValue(cbConverter), {
  ...
  _SetValue: function (newValue, event, options) {
    ...
    if (this._isContainedInDateTimePicker()) {
      ...
      try {
        ...
        if (
          parsedNewValue &&
          IntlConverterUtils._compareISODates(dateTimeValue, parsedNewValue) === 0
        ) {
  ...
  _getTimePositioning: function (ptrn) {
    ...
    const mainNodeKey = OraI18nUtils.getLocaleElementsMainNodeKey(localeElements);
  ...
  _getIsoDateLimit: function (converter, optionName, valueDate) {
    ...
    dateIso = dateIso ? IntlConverterUtils._minMaxIsoString(dateIso, this._getValue()) : dateIso;
  ...
  _getAmPmStrings: function () {
    var converter = new IntlDateTimeConverter({ pattern: 'hh:a' });
  ...
  _createWheelPickerDom: function (keepHeaderFooter) {
    ...
    try {
      ...
      info('The value of the InputTime element must be an ISOString, please use a valid ISOString');
      value = IntlConverterUtils.dateToLocalIso(date);
      valueDate = converterUtils._clearTime(value);
    }
    ...
    if (pattern === undefined) {
      ...
      let newConverter = new IntlDateTimeConverter(resolvedOption);
      let newOptions = newConverter.resolvedOptions();
      pattern = newOptions.pattern || newOptions.patternFromOptions;
      warning(`time component's converter.resolvedOptions has no pattern or patternFromOptions
  ...
  _isTimePickerSupported: function () {
    return (
      ...
      (_showPickerOnDesktop === 'disabled' && getDeviceRenderMode() !== 'others')
    );
  },
  /**
...
setDefaultOptions({
  ojInputTime: {
    ...
    timePicker: createDynamicPropertyGetter(function () {
...
oj.__registerWidget('oj.ojInputDateTime', $.oj.ojInputDate, {
        ...
        switchDone: function (evt) {
          ...
          if (
            ...
            if (self._switcherDateValue) {
              ...
              if (dateIso && IntlConverterUtils._getISOStrFormatType(dateIso) === 'local') {
                newVal = dateIso;
              } else {
  ...
  _updateSwitcherText: function () {
    ...
    if (this._isShowingDatePickerSwitcher()) {
      ...
      const dateSwitcherConverter = new IntlDateTimeConverter({
        day: '2-digit',
        month: '2-digit',
        year: _yearFormat$1,
        timeZone,
      ...
      try {
        ...
        const timeSwitcherConverter = new IntlDateTimeConverter({
          hour: '2-digit',
          minute: '2-digit'
        });
  ...
  getValueForInputTime: function () {
    ...
    if (this._isDateTimeSwitcher()) {
      ...
      if (this._switcherTimeValue && value) {
        value = IntlConverterUtils._copyTimeOver(this._switcherTimeValue, value);
      }
    }

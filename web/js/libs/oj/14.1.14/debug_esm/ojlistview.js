...
import { enableAllFocusableElements, isFirefox, isMobileTouchDevice, isElementIntersectingScrollerBounds, disableAllFocusableElements, getBrowserVersion, isIE, isElementOrAncestorFocusable, isWindows, isEventClickthroughDisabled, handleActionablePrevTab, handleActionableTab, getFocusableElementsInNode, isEdge, disableDefaultBrowserStyling } from 'ojs/ojdatacollection-common';
...
const _ojListView = _ListViewUtils.clazz(
  ...
  /** @lends oj._ojListView.prototype */ {
    ...
    init: function (opts) {
      ...
      if (isFirefox() && this._isComponentFocusable()) {
    ...
    CreateDataContentHandler: function (data) {
      ...
      if (typeof oj.TableDataSource !== 'undefined' && data instanceof oj.TableDataSource) {
        ...
        contentHandler = new IteratingDataProviderContentHandler(
          this,
          this.element[0],
    ...
    _getShowStatusDelay: function () {
      ...
      var delay = getCSSTimeUnitAsMillis(defaultOptions.showIndicatorDelay);
    ...
    _setScrollY: function (scroller, y) {
      ...
      info('Setting scroll y: ' + y + ' initial scrollTop: ' + initialScrollTop);
      ...
      info('scrollTop after updating: ' + scroller.scrollTop);
      if (initialScrollTop === scroller.scrollTop) {
        info('scrollPosBusyState getting clear');
        this._clearScrollPosBusyState();
        return false;
      }
      return true;
    ...
    _validateKeyForScroll: function (key, skipLocal) {
      ...
      if (
        this.m_contentHandler instanceof IteratingDataProviderContentHandler
      ) {
        var dataProvider = this.m_contentHandler.getDataProvider();
    ...
    _cleanupTabbableElementProperties: function (elem) {
      enableAllFocusableElements(elem[0]).forEach(function (node) {
    ...
    _supportRelatedTargetOnBlur: function () {
      if (isFirefox() && getBrowserVersion() < 48) {
        return false;
      }
    ...
    _isNodeFocusable: function (node) {
      ...
      return isElementOrAncestorFocusable(node, function (element) {
        return (
    ...
    _isNonWindowTouch: function () {
      return this._isTouchSupport() && !isWindows();
    },
    ...
    _isClickthroughDisabled: function (event, elem) {
      return elem.length > 0 && isEventClickthroughDisabled(event, elem.get(0));
    },
    ...
    _registerScrollHandler: function () {
      ...
      this._scrollListener = function (event) {
        ...
        info('scroll listener: ' + self._skipScrollUpdate);
        if (self._skipScrollUpdate) {
          info('clearing scrollPosBusyState');
